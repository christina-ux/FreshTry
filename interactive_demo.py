...
with st.sidebar:
    ...
    if st.session_state.logged_in:
        page = st.radio("", ["Dashboard", "Upload Policy", "Analyze Policy", "Reports", "Settings", "Logout"])
    else:
        page = st.radio("", ["Home", "Login", "Register", "Samples"])
...
if page == "Home":
    ...
    st.markdown("### AI-powered policy document analysis")
    ...
    with col1:
        ...
        - Analyze and improve policy documents
        - Check compliance with regulations
        - Enhance readability and clarity
        - Compare against industry benchmarks
        
        We use advanced AI to scan your policies and provide actionable insights.
        """)
        
        if st.button("Try Demo Analysis", type="primary"):
            st.session_state.logged_in = True
            page = "Upload Policy"
            st.experimental_rerun()
    
    with col2:
        st.image("https://via.placeholder.com/400x300?text=PolicyEdgeAI", use_column_width=True)
    ...
    with col1:
        ...
        st.markdown("Upload your policy documents in various formats")
        st.markdown('</div>', unsafe_allow_html=True)
    
    with col2:
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.markdown("#### 2. Analyze")
        st.markdown("Our AI scans for compliance, clarity, and completeness")
        st.markdown('</div>', unsafe_allow_html=True)
    
    with col3:
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.markdown("#### 3. Improve")
        st.markdown("Get actionable recommendations to enhance your policies")
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    if st.button("Access Demo Account", type="secondary"):
    ...
    for policy in st.session_state.sample_policies:
        ...
        with col4:
            if st.button("View", key=f"view_{policy['name']}"):
    ...
    with sample_tabs[0]:
        ...
        if st.button("Analyze Sample Privacy Policy"):
    ...
    with sample_tabs[1]:
        ...
        if st.button("Analyze Sample Terms of Service"):
    ...
    with sample_tabs[2]:
        ...
        if st.button("Analyze Sample Cookie Policy"):
            st.info("This would show an analysis similar to the Privacy Policy example.")
